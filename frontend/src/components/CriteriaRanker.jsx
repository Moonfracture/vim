  ...
  return (
    <div>
      {order.length > 0 ? (
        ...
        <div className="rounded-xl border border-dashed border-forest/20 px-4 py-5 text-center text-sm text-forest/50">
      ...
      {unused.length > 0 && (
        <div className="mt-3">
          <p className="mb-2 text-[11px] uppercase tracking-wider text-forest/50">Добави критерий</p>
          <div className="flex flex-wrap gap-2">
            {unused.map((c) => (
              <button
                ...
                type="button"
                onClick={() => add(c.id)}
                className="chip transition-colors hover:border-accent/40 hover:text-forest-ink"
              >
  ...
  return (
    <Reorder.Item
      ...
      className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 ${
        dragging ? 'border-accent/60 bg-accent/10' : 'border-forest/10 bg-ink-850'
      }`}
    >
      ...
      <span className="text-forest/60"><c.icon size={18} /></span>
      <span className="flex-1 text-sm font-medium text-forest-ink">{c.label}</span>
      <button
        type="button"
        onClick={onRemove}
        className="text-forest/50 transition-colors hover:text-coral"
      ...
      <button
        ...
        className="cursor-grab touch-none text-forest/50 transition-colors hover:text-forest/70 active:cursor-grabbing"
        aria-label="Влачи"
      >
        <Icon.drag size={18} />
      </button>
